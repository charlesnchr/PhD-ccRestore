...
def EvaluateModel(opt):
    ...
    if opt.stats_tubule_sheet:
        if opt.out == 'root':
            fid = open('%s/stats_tubule_sheet.csv' % opt.root,'w')
        else:
            fid = open('%s/stats_tubule_sheet.csv' % opt.out,'w')
        fid.write('Filename,Tubule fraction,Sheet fraction,Tubule/sheet ratio\n')
    ...
    for i, imgfile in enumerate(imgs):
        ...
        if opt.stats_tubule_sheet:
            ...
            npix = w*h
            fid.write('%s,%0.4f,%0.4f,%0.4f\n' % (os.path.basename(imgfile),fraction1/npix,fraction2/npix,fraction1/fraction2))
        if opt.weka_colours:
        ...
        if opt.out == 'root': # save next to orignal
            ...
            Image.fromarray(oimg).save('%s/%04d.png' % (opt.out,i))
