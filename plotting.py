...
def testAndMakeCombinedPlots(net, loader, opt, idx=0):
        ...
        for j in range(len(lr_bat)):  # loop over batch
            ...
            if opt.task == "segment":
                ...
                if i + 1 == len(loader):
                    print(
                        "successes (%d/%d)"
                        % (mean_sr_psnr, mean_bc_psnr + mean_sr_psnr)
                    )
                ...
                if "sim" not in opt.dataset and opt.scale > 1:
                    ...
                    if lr.shape[0] > 3:
                        lr = lr[
                            lr.shape[0] // 2
                        ]  # channels are not for colours but separate grayscale frames, take middle
                        hr = hr[hr.shape[0] // 2]
                    ...
                    if lr.shape[0] == 1:
                        ...
                        bc = ndimage.gaussian_filter(
                            img, sigma=(0.5, 0.5, 0.2), order=0
                        )
            ...
            if makeplotBool and not opt.test:
                ...
                plt.savefig(
                    "%s/combined_epoch%d_%d.png" % (opt.out, idx + 1, count),
                    dpi=300,
                    bbox_inches="tight",
                    pad_inches=0,
                )
            ...
            if not opt.disable_wandb:  # wandb imaging logging
                if opt.task == "segment":
                    opt.wandb.log(
                        {"valid_img_lr_%d" % count: wandb.Image(lr)}, step=idx + 1
                    )
                    opt.wandb.log(
                        {"valid_img_sr_%d" % count: wandb.Image(sr)}, step=idx + 1
                    )
                    opt.wandb.log(
                        {"valid_img_hr_%d" % count: wandb.Image(hr)}, step=idx + 1
                    )
                else:
                    opt.wandb.log(
                        {"valid_img_lr_%d" % count: wandb.Image(lr)}, step=idx + 1
                    ...
                    opt.wandb.log(
                        {"valid_img_bc_%d" % count: wandb.Image(bc)}, step=idx + 1
                    )
                    opt.wandb.log(
                        {"valid_img_sr_%d" % count: wandb.Image(sr)}, step=idx + 1
                    )
    ...
    if count == 0:
        ...
        count = 1
    summarystr += (
        "Testing of %d samples complete. bc: %0.2f dB / %0.4f, sr: %0.2f dB / %0.4f"
        % (
            count,
            mean_bc_psnr / count,
            mean_bc_ssim / count,
    ...
    if not opt.disable_wandb:
        opt.wandb.log(
            {
                "valid_bc_psnr": mean_bc_psnr / count,
                "valid_bc_ssim": mean_bc_ssim / count,
                "valid_sr_psnr": mean_sr_psnr / count,
                "valid_sr_ssim": mean_sr_ssim / count,
            },
            step=idx + 1,
        )
    print(summarystr)
