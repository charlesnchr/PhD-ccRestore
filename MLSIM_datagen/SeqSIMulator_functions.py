...
def PsfOtf(w, scale):
    ...
    yy = (
        np.abs(2 * scipy.special.jv(1, scale * R + eps) / (scale * R + eps)) ** 2
    )  # 0.5 is introduced to make PSF wider
    yy0 = fftshift(yy)
...
def Generate_SIM_Image(opt, Io, in_dim=512, gt_dim=1024):
    ...
    if opt.OTF_and_GT:
        ...
        gt_img = Io[:, :, opt.Nangles * opt.Nshifts // 2].astype(
            "float"
        )  # center frame

        if type(gt_dim) is int:
            gt_img = transform.resize(
                gt_img, (gt_dim, gt_dim), anti_aliasing=True, order=3
            )
        else:
            gt_img = transform.resize(gt_img, gt_dim, anti_aliasing=True, order=3)
