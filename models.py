...
def GetModel(opt):
    if opt.model.lower() == "edsr":
        ...
        net = GeneratorRRDB(
            opt, filters=opt.n_feats, num_res_blocks=opt.n_resblocks, num_upsample=0
        )
...
def normalizationTransforms(
    normtype,
):  ## NB is this even right? I think normalize and unnormalize should be flipped
    if normtype.lower() == "div2k":
        normalize = MeanShift(1, [0.4485, 0.4375, 0.4045], [0.2436, 0.2330, 0.2424])
        unnormalize = MeanShift(
            1, [-1.8411, -1.8777, -1.6687], [4.1051, 4.2918, 4.1254]
        )
        print("using div2k normalization")
    elif normtype.lower() == "pcam":
        normalize = MeanShift(1, [0.6975, 0.5348, 0.688], [0.2361, 0.2786, 0.2146])
        unnormalize = MeanShift(
            1, [-2.9547, -1.9198, -3.20643], [4.2363, 3.58972, 4.66049]
        )
...
class ResBlock2Max(nn.Module):
    def __init__(
        ...
        m.append(
            nn.ConvTranspose2d(
                4 * n_feats, 2 * n_feats, 3, stride=2, padding=1, output_padding=1
            )
        )

        m.append(
            nn.ConvTranspose2d(
                2 * n_feats, n_feats, 3, stride=2, padding=1, output_padding=1
            )
        )
...
class ResBlock3Max(nn.Module):
    def __init__(
        ...
        m.append(
            nn.ConvTranspose2d(
                8 * n_feats, 4 * n_feats, 3, stride=2, padding=1, output_padding=1
            )
        )
        m.append(
            nn.ConvTranspose2d(
...
class EDSR2Max(nn.Module):
    def __init__(self, normalization=None, nch_in=3, nch_out=3, scale=4):
        ...
        # define body module
        m_body = [
            ResBlock2Max(conv, n_feats, kernel_size, act=act, res_scale=0.1)
            for _ in range(n_resblocks)
        ]
        m_body.append(conv(n_feats, n_feats, kernel_size))

        # define tail module
        m_tail = [conv(n_feats, nch_out, kernel_size)]
...
class EDSR3Max(nn.Module):
    def __init__(self, normalization=None, nch_in=3, nch_out=3, scale=4):
        ...
        # define body module
        m_body = [
            ResBlock3Max(conv, n_feats, kernel_size, act=act, res_scale=0.1)
            for _ in range(n_resblocks)
        ]
        m_body.append(conv(n_feats, n_feats, kernel_size))
...
class RNAN(nn.Module):
    def __init__(self, opt):
        ...
        # define body module
        modules_body_nl_low = [
            _NLResGroup(conv, n_feats, kernel_size, act=act, res_scale=1)
        ]
        modules_body = [
            _ResGroup(conv, n_feats, kernel_size, act=act, res_scale=1)
