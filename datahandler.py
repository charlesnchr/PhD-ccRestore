...
normalize2 = transforms.Normalize(
    mean=[0.69747254, 0.53480325, 0.68800158], std=[0.23605522, 0.27857294, 0.21456957]
)
unnormalize2 = transforms.Normalize(
    mean=[-2.9547, -1.9198, -3.20643], std=[4.2363, 3.58972, 4.66049]
)
...
def GetDataloaders(opt):
    ...
    if opt.dataset.lower() == "div2k":
        ...
        dataloader = load_HDF5_dataset(
            opt.root + "/camelyonpatch_level_2_split_valid_x.h5", "train", opt
        )
        validloader = load_HDF5_dataset(
            opt.rootValidation + "/camelyonpatch_level_2_split_valid_x.h5", "valid", opt
        )
        ...
        dataloader = load_ER_dataset(
            opt.root, category="train", batchSize=opt.batchSize, num_workers=opt.workers
        )
        validloader = load_ER_dataset(
            opt.rootValidation,
            category="valid",
            shuffle=False,
...
class ImageDataset(Dataset):
    def __init__(self, root, category, opt):
        ...
        self.croptransform = transforms.Compose(
            [transforms.RandomCrop(opt.imageSize * opt.scale), transforms.ToTensor()]
        )
        self.scaletransform = transforms.Compose(
            [
                transforms.ToPILImage(),
...
def load_image_dataset(root, category, opt):
    ...
    if category == "train":
        ...
        dataloader = DataLoader(
            dataset, batch_size=opt.batchSize_test, shuffle=False, num_workers=0
        )
    return dataloader
...
class DIV2KDataset(Dataset):
    ...
    ):  # highres images not currently scaled, optdefault
    ...
    def __getitem__(self, index):
        ...
        hq = hq.resize(
            (self.scale * self.imageSize, self.scale * self.imageSize), Image.BICUBIC
        )
...
class PickleDataset(Dataset):
    def __init__(
        ...
        self.images = glob.glob(
            ...
            + "/DIV2K_"
            + category
            + "_HR_bins%dx%d/*" % (opt.imageSize, opt.imageSize)
        )
    ...
    def __getitem__(self, index):
        if self.scale > 1:  # SR
            ...
            if len(inputTuple) == 2:
                ...
                len(inputTuple) == 4
            ):  ## assuming time sequence of 3 adjacent frames for input
            ...
            img_in_pre, img_in, img_in_pos, hq = (
                toTensor(img_in_pre).float(),
                toTensor(img_in).float(),
                toTensor(img_in_pos).float(),
                toTensor(img_gt).float(),
            )
...
class SIM_dataset(Dataset):
    def __init__(self, root, category, opt):
        ...
        self.targetimages = (
            mat["simdata"].reshape(xdim, ydim, 1, nsamples).transpose(3, 0, 1, 2)
        )  # no channels by default
        self.gtimages = (
            mat["gtdata"].reshape(xdim, ydim, 1, nsamples).transpose(3, 0, 1, 2)
        )  # no channels by default
...
class real_SIM_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        targetimg = np.vstack((toprow, botrow)).reshape(
            2 * mat.shape[0], 2 * mat.shape[1], 1
        )
        ...
        if self.scale == 1:
            ...
            targetimg = toTensor(
                targetimg.resize((inputimg.shape[2], inputimg.shape[1]), Image.BICUBIC)
            )
...
class Fourier_SIM_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if "subset" in self.task and self.nch_in == 6:
            ...
            "subset" in self.task and self.nch_in == 1
        ):  # don't do it if widefield input is expected
            inputimg = stack[
                [8]
            ]  # used for sequential SIM - first tests from 20201215 have GT as 9th frame
            ...
            "last" in self.task and self.nch_in == 1
        ):  # don't do it if widefield input is expected
            inputimg = stack[
                [8]
            ]  # used for sequential SIM - first tests from 20201215 have GT as 9th frame
        ...
        if len(stack) > 9:
            ...
            if self.scale == 2:
                ...
                gt = np.vstack((toprow, botrow)).reshape(
                    2 * stack.shape[1], 2 * stack.shape[2]
                )
            elif self.nch_out > 1:
        ...
        if self.norm == "adapthist":
            ...
            simimg = (simimg - torch.min(simimg)) / (
                torch.max(simimg) - torch.min(simimg)
            )
...
class ERdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        noisyimg = Z * img
        noisyimg = (
            0.2 * np.random.rand() + 0.8
        ) * noisyimg  # overall level between 0.5 and 0.1
